...
setenv('ROS_MASTER_URI','http://172.29.29.80:11311') % IP del entorno del robot real
...
laser = rossubscriber('/robot0/laser_1','sensor_msgs/LaserScan');
pause(2);
%% DECLARACIÓN DE PUBLISHERS 
pub = rospublisher('/robot0/cmd_vel', 'geometry_msgs/Twist'); 

%% Código para Simulador STDR: Sensor Láser

% Se asume que ya se ha inicializado ROS y se han configurado los topics correspondientes.
% En simulación, el sensor láser se suele publicar en '/robot0/laser_1'.
...
pause(5); % Pequeña pausa para asegurar conexión

while isempty(laser.LatestMessage) % Esperar hasta recibir datos del láser
    pause(0.1); % Pequeño retardo para no sobrecargar el bucle
end
...
for i = 1:numMediciones
    ...
    if isempty(scanMsg) || isempty(scanMsg.Ranges)
        rawDistanceData(i) = NaN;  % Si no hay datos, guardar NaN
    else
